...
export function CalendarWidget({ tasks = [] }: CalendarWidgetProps) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        key={day}
        className={`h-6 w-6 sm:h-8 sm:w-8 rounded-lg flex items-center justify-center text-xs sm:text-sm relative cursor-pointer transition-all duration-200 ${
          isToday
      ...
      >
        ...
        {hasEvent && (
          <div className={`absolute -bottom-1 w-1 h-1 rounded-full ${
            eventForDay?.type === "deadline" ? "bg-priority-high" :
